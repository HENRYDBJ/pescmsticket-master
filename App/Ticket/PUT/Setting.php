...
class Setting extends \Core\Controller\Controller {
    ...
    public function atUpgrade() {
        $getPatch = json_decode((new \Expand\cURL())->init('https://www.1pescms.com/patch/5/'.\Core\Func\CoreFunc::$param['system']['version'], [], [
            CURLOPT_HTTPHEADER => [
        ...
        if($getPatch['status'] == 200){
            ...
            $getFile = (new \Expand\cURL())->init("https://www.1pescms.com{$getPatch['data']['update_patch_file']}");
    ...
    public function mtUpgrade() {
        ...
        if(hash_file('sha256', $file['tmp_name']) !== $getPatch['data']['patch_sha256']){
            $this->error('非官方更新补丁!请访问<a href="https://www.1pescms.com" target="_blank">PESCMS</a>获取最新的补丁', 'javascript:history.go(-1)', '10');
        }
